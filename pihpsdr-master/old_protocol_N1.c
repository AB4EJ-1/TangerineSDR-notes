...
#define PMODE 0644   // for raw binary output
...
static	int num_subchannels = 2; /* subchannels */
static	int marching_periods = 1; /* no marching periods when writing */
static	char uuid[100] = "pihpsdr output";
static  char hdf5File[50] = "/tmp/ramdisk/ch0_3";
static  char hdf5File1[50] = "/tmp/ramdisk/ch4_7";
...
static	uint64_t vector_length = 36; /* number of samples written for each call -  */
static  uint64_t vector_sum = 0;
static  int fd;  // may be used EITHER for Digital RF or UDP bcast output
static  struct sockaddr_in addr;
static  bool oneTime = true;
static unsigned long time_in_microsec;
static struct timeval tv;
static unsigned long long avg_time;
static long timing_counter;
...
void old_protocol_init(int rx,int pixels,int rate) {
  ...
  fprintf(stderr,"(N) old_protocol_init\n");
  hdf_i = 0;  // wde
   vector_leading_edge_index = 0;  //wde
  fprintf(stderr,"Digital RF init\n");
    ...
    theUnixTime = (unsigned long)time(NULL);
    printf("Start (Unix) time: %li",theUnixTime);

	/* start recording at global_start_sample (original = 1394368230 ) */
	global_start_index = (uint64_t)(theUnixTime * (long double)sample_rate_numerator/sample_rate_denominator) + 1; /* should represent 2014-03-09 12:30:30  and 10 milliseconds*/

// experimental code for setting up to broadcast IQ data
    fprintf(stderr, "set up UDP bcast socket\n");
  ...
  //  struct sockaddr_in addr;
    ...
    int bytes_sent = sendto(fd, message, len, 0, (struct sockaddr *) &addr, sizeof(addr));
    fprintf(stderr, "bcast complete\n");
    sampleCounter = 0; 

/*
	printf("Writing complex to multiple files and subdirectores in \n");
    printf("%s",hdf5File);
    printf("\n");
    sysCommand1 = "rm -rf ";
    char * sysCommand = (char *) malloc(1 + strlen(sysCommand1) + strlen(hdf5File) + 6);
    strcpy(sysCommand, sysCommand1);
    strcat(sysCommand, hdf5File);
   // printf("%s",sysCommand);
    ...
    strcat(sysCommand, hdf5File);
    printf("create: \n");
    printf("%s",sysCommand); printf("\n");
    result = system(sysCommand);
    sysCommand2 = "/junk0";
    printf("create: \n");
    strcat(sysCommand, sysCommand2);
    printf("%s",sysCommand); printf("\n");
    result = system(sysCommand);
*/
    ...
    //result = system("rm -rf /media/odroid/hamsci/hdf5 ; mkdir /media/odroid/hamsci/hdf5 ; mkdir /media/odroid/hamsci/hdf5/junk0");

/*
	  // H5T_NATIVE_FLOAT - should be 4 bytes per sample 
	data_object = digital_rf_create_write_hdf5(hdf5File, H5T_NATIVE_FLOAT , subdir_cadence,      millseconds_per_file,
...
/*
	data_object1 = digital_rf_create_write_hdf5(hdf5File1, H5T_NATIVE_FLOAT , subdir_cadence, millseconds_per_file,
			global_start_index, sample_rate_numerator, sample_rate_denominator, uuid, compression_level, checksum, is_complex, num_subchannels,
			is_continuous, marching_periods);
*/
/*
...
*/
   ...
   // fd = creat(filename, PMODE);  // for testing raw binary output

  	fprintf(stderr,"hdf5 dataset initialized");
...
//
  ...
  fprintf(stderr,"(N) old_protocol_init: prime radio\n");
...
void *write_digital_rf( void *ptr)
{
   // char *message;
   // message = (char *) ptr;
    printf("thread call \n");
...
static void process_ozy_input_buffer(unsigned char  *buffer) {
  ...
  pthread_t thread1; // WDE
  int iret1;  // WDE
  const char *message1 = "Trigger thread";
  gettimeofday(&tv,NULL);
  if(time_in_microsec == 0)
        {
        time_in_microsec = 1000000 * tv.tv_sec + tv.tv_usec;
        }           
    else
        {
        time_in_microsec = 1000000 * tv.tv_sec + tv.tv_usec - time_in_microsec;
        //fprintf(stderr,"t = %lu", time_in_microsec);
        avg_time = avg_time + time_in_microsec;
        timing_counter++;
        time_in_microsec = 1000000 * tv.tv_sec + tv.tv_usec;
        }       
  ...
  if(buffer[b++]==SYNC && buffer[b++]==SYNC && buffer[b++]==SYNC) {
    ...
    int iq_samples=(512-8)/((num_hpsdr_receivers*6)+2);  // num receivers is 2 for Red Pitaya
  //  sampleCounter = 0;  // TODO:  CHECK - THIS MAY NEED TO BE ONLY AT TOP
...
/*
      // for(int k = 0; k<4; k++)  // use 32 for 16 channels
            {
	        data_hdf5[sampleCounter]     = left_sample_double;  // for digital rf
	        data_hdf5[sampleCounter + 1] = right_sample_double;
//fprintf(stderr,"%i %g   %g \n", sampleCounter, data_hdf5[sampleCounter], data_hdf5[sampleCounter +1]);
            sampleCounter = sampleCounter + 2;
            }
/*
                    {  // simulate additional receiver 
	                data_hdf5[sampleCounter]     = 0.0;  // for digital rf
	                data_hdf5[sampleCounter + 1] = 0.0;
                    sampleCounter = sampleCounter + 2;
                    } 
...
/*
 ...
 //printf("save vector ctr=%lli \n",sampleCounter + 3 * vector_length);
// for simulating 4x the sample rate
/*
	    data_hdf5[sampleCounter][2] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][3] = right_sample_double;
	    data_hdf5[sampleCounter][4] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][5] = right_sample_double;
	    data_hdf5[sampleCounter][6] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][7] = right_sample_double;
	    data_hdf5[sampleCounter][8] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][9] = right_sample_double;
	    data_hdf5[sampleCounter][10] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][11] = right_sample_double;
	    data_hdf5[sampleCounter][12] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][13] = right_sample_double;
	    data_hdf5[sampleCounter][14] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][15] = right_sample_double;
*/
    ...
    } // end of loop for(i=0;i<iq_samples;i++) 

    // digital RF /////////////////////////////////////////////////////////////////////////////////
  // fprintf(stderr," Skipping DRF code \n");
/*
...
*/
 ...
 //   vector_sum = vector_leading_edge_index + hdf_i*vector_length*8; 
  //  result = digital_rf_write_hdf5(data_object, vector_sum, data_hdf5,vector_length); 
   // fprintf(stderr,"t = %lu   \n", timing_counter);
    if(timing_counter > 10000)
        {
        avg_time = avg_time / 10000;
        fprintf(stderr,"avg t = %llu \n", avg_time);
        timing_counter = 0;
        avg_time = 0; 
        }
  //  hdf_i++;
...
/*
    if(sampleCounter > vector_length)  
     {
     for(int z=0;z<4;z++)  // repeat 4 times to simulate 48 ksps X 4 = 192 ksps
      {
      vector_sum = vector_leading_edge_index + hdf_i*vector_length*8*16; 

// buffer size: vector length X 8 (bytes per complex sample) X 8 (subchannels)
      //fprintf(stderr,"%i  Vector Length = %lli \n", z,vector_length * 8 * 8); 
      
      result = digital_rf_write_hdf5(data_object, vector_sum, data_hdf5,vector_length); 
     // iret1 = pthread_create( &thread1, NULL, write_digital_rf, (void*) message1);
    //  result = digital_rf_write_hdf5(data_object1, vector_sum, data_hdf5, vector_length);

      //result= digital_rf_close_write_hdf5(data_object);

     //  n = write(fd, data_hdf5, vector_length * 8 * 8 );  // to simulate 182 ksps
     //  n = write(fd, data_hdf5, vector_length * 8 * 8 );
     //  n = write(fd, data_hdf5, vector_length * 8 * 8 );
    //   n = write(fd, data_hdf5, vector_length * 8 * 8 );
    //   fprintf(stderr,"%i \n", n);
