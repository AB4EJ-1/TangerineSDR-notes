...
static  int sampleCounter = 0;
	/*  dataset to write */
static	float data_hdf5[8000][2];

	/* writing parameters */
static	uint64_t sample_rate_numerator = 48000; /* 100 Hz sample rate - typically MUCH faster */
static	uint64_t sample_rate_denominator = 1;
static	uint64_t subdir_cadence = 4; /* Number of seconds per subdirectory - typically longer */
static	uint64_t millseconds_per_file = 400; /* Each subdirectory will have up to 10 400 ms files */
static	int compression_level = 1; /* low level of compression */
...
static	int num_subchannels = 1; /* only one subchannel */
static	int marching_periods = 1; /* no marching periods when writing */
static	char uuid[100] = "pihpsdr output";
static	uint64_t vector_length = 6400; /* number of samples written for each call - typically MUCH longer */
...
void old_protocol_init(int rx,int pixels,int rate) {
  ...
  fprintf(stderr,"Digital RF init\n");
	...
	/* init */
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk0", H5T_NATIVE_FLOAT, subdir_cadence, millseconds_per_file,
  ...
  if(buffer[b++]==SYNC && buffer[b++]==SYNC && buffer[b++]==SYNC) {
    ...
    for(i=0;i<iq_samples;i++) {
      for(r=0;r<num_hpsdr_receivers;r++) {
	    ...
	    data_hdf5[sampleCounter][0] = left_sample_double;  // for digital rf
	    data_hdf5[sampleCounter][1] = right_sample_double;
        sampleCounter++;  // later, we'll check to see if buffer full
    ...
    if(sampleCounter > 6400) {
      vector_sum = vector_leading_edge_index + hdf_i*6400; 
      //fprintf(stderr,"write digital rf %i\n", vector_sum); 
