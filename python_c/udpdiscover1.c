...
void UDPdiscover(char *data, int *LH_port) {
  ...
  old_discovery();
  printf("DISCOVER COMPLETED\n");
  printf("passing back port# %i\n",discovered[0].info.network.address.sin_port); 
  sleep(1);
  *LH_port = discovered[0].info.network.address.sin_port;  // selected port back via reference
...
//  memcpy(LH_port, &selected_port, sizeof(selected_port));
  ...
  memcpy(data, discovered[0].name,14);
